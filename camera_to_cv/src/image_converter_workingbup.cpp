...
double gamma_hole = 0;
int brightness_hole = -1015;
int contrast_hole = 7;
double gamma_object = 0;
int brightness_object = -801;
int contrast_object = 4;
...
int main(int argc, char** argv)
{
  ...
  while(ros::ok())
  {
    if(cv_ptr)
    {
      ...
      threshold(im_objects_redthr, im_bw_objects_fromred, 100, 255.0, THRESH_BINARY_INV);
      threshold(im_objects_greenthr, im_bw_objects_fromgreen, 100, 255.0, THRESH_BINARY);
